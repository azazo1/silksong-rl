# 编译 RL Env (丝之歌强化学习训练用 BepInEx 插件).
#
...
#   pwsh -File mods/rl-env/build.ps1 -Install -GameDir 'D:\games\steam\common\Hollow Knight Silksong'
#
# -Install 会把编译结果复制到 <游戏目录>/BepInEx/plugins/RLEnv/.
...
$outputDll = Join-Path $outputDir 'RLEnv.dll'
$sourceDir = Join-Path $root 'src'
$pluginFolderName = 'RLEnv'

function Write-Step {
    param([string]$Message)

    Write-Host "[rl-env] $Message"
}
...
$references = @(
    ...
    (Join-Path $managedDir 'PlayMaker.dll'),
...
Copy-Item -LiteralPath $outputDll -Destination (Join-Path $pluginDir 'RLEnv.dll') -Force
Write-Step "已安装插件: $pluginDir"

# Boss 清单与存档与 mods/boss-rush 共用一套资源: 仓库里存的是 gzip 过的 JSON,
# 安装期由 boss-rush 的编解码脚本还原成游戏认的 .dat.
$bossRushRoot = Join-Path (Split-Path $root -Parent) 'boss-rush'
$sourceScenes = Join-Path $bossRushRoot 'BossScenes'
if (-not (Test-Path -LiteralPath $sourceScenes)) {
    throw "找不到 Boss 资源目录: $sourceScenes"
}
...
. (Join-Path $bossRushRoot 'tools\SaveCodec.ps1')
...
Write-Step "已铺设 Boss 清单与 $decoded 个存档: $targetScenes"
